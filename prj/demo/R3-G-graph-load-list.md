...
The demo project SHALL provide `--load` and a solo `--list`
action for graph storage. The tool SHALL also accept an
`--id` argument for graph selection. The loader SHALL be
able to load a graph from a simple JSON file such as
`001-graph.json`, and SHALL be able to list loaded graphs.
The loader SHALL use the file name to determine the graph
ID, so `001-graph.json` loads as graph ID `1`. If graph ID
`1` already exists in the database, the loader SHALL delete
existing rows for that graph ID before inserting the new
