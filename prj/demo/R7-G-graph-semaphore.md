...
The demo project SHALL provide a semaphore mechanism so
multiple PID processes can coordinate work on the same graph
without conflicting updates. The semaphore SHALL accept a
PID argument so a process can identify itself when claiming
or releasing access.
