...
def ensure_scientist_mode_support(cur):
    ...
    row = cur.execute("select 1 from metacognitive_state where state_key=?", (SCIENTIST_STATE_KEY,)).fetchone()
    if not row:
        cur.execute(
            '''
            ...
            VALUES(?,?,?,?,?,?)
            ''',
            (SCIENTIST_STATE_KEY, 'roles', 'general', 1.0, 'system', 1),
        )


def ensure_route_mode_support(cur):
    cur.execute(
        '''
        INSERT INTO feature_flags(feature_key, enabled, switchable, scope, updated_by)
        VALUES(?,?,?,?,?)
        ...
        (
            ...
            0,
            1,
            'Route-recognition mode: enables route matching in normal chat input.',
            'system',
        ),
    )
    row = cur.execute("select 1 from metacognitive_state where state_key=?", (ROUTE_STATE_KEY,)).fetchone()
    if not row:
        cur.execute(
            '''
            INSERT INTO metacognitive_state(state_key, category, value, confidence, provenance, version)
            VALUES(?,?,?,?,?,?)
            ''',
            (ROUTE_STATE_KEY, 'modes', 'general', 1.0, 'system', 1),
        )
...
def scientist_status(conn):
    ...
    row = cur.execute(
        ...
        ''',
        (SCIENTIST_STATE_KEY, SCIENTIST_FLAG_KEY),
    ).fetchone()
    if not row:
...
def toggle_route_mode(conn, enabled, actor='Peter', reason='Explicit mode command'):
    cur = conn.cursor()
    ...
    if current == enabled:
        ...
        return {
            'feature_key': ROUTE_FLAG_KEY,
            'enabled': bool(current),
            'changed': False,
            'active_route_mode': role,
            'message': f'route mode already {"on" if enabled else "off"}',
        }
    ...
    return {
        ...
        'changed': True,
        'active_route_mode': role,
        'message': f'route mode {"enabled" if enabled else "disabled"}',
    }
...
def route_status(conn):
    ...
    row = cur.execute(
        ...
        where ff.feature_key=?
        ''',
        (ROUTE_STATE_KEY, ROUTE_FLAG_KEY),
    ).fetchone()
    if not row:
        raise RuntimeError('route mode support missing')
    enabled, switchable, updated_by, updated_at, role_value, role_confidence, role_version = row
    return {
        'feature_key': ROUTE_FLAG_KEY,
        'enabled': bool(enabled),
        ...
        'updated_by': updated_by,
        'updated_at': updated_at,
        'active_route_mode': role_value,
        'active_route_confidence': role_confidence,
        'active_route_version': role_version,
    }
    ...
    try:
        if role == 'scientist':
            ...
            if action == 'on':
                ...
                result = toggle_route_mode(conn, 0)
            elif action == 'status':
