...
class TDD_CodeEthicsTests(unittest.TestCase):
    ...
    def test_minimal_tier_only_enforces_hard_gates(self):
        ...
        try:
            continuity_db_helper.cmd_run(argparse.Namespace(name="ethics_minimal_art", db=db, timeout=1, program_args=[]))
    ...
    def test_full_tier_enforces_all_checks(self):
        ...
        try:
            continuity_db_helper.cmd_run(argparse.Namespace(name="ethics_full_art", db=db, timeout=1, program_args=[]))
        except SystemExit as e:
            # If it fails, it should NOT be due to ethics check failure
    ...
    def test_minimal_tier_skips_non_hard_gates(self):
        ...
        try:
            continuity_db_helper.cmd_run(argparse.Namespace(name="ethics_minimal_skip", db=db, timeout=1, program_args=[]))
        except SystemExit as e:
