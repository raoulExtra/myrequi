...
async def receive_one_async(bot: Any) -> dict[str, Any]:
    """Receive and acknowledge one pending Telegram update without local storage."""
    ...
    if update_id is not None:
        # Telegram acknowledges updates below this offset server-side. Any
        # newer update returned by this call remains pending for next receive.
        await bot.get_updates(offset=int(update_id) + 1, limit=1)
    return {
        "update_id": update_id,
...
def receive_one(bot: Any, db_path: str | Path | None = None) -> dict[str, Any]:
    """Expose one update; db_path is accepted only for executor compatibility."""
    del db_path
