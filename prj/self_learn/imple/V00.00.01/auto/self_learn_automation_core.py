...
def docs_index(root: Path = PROJECT_ROOT) -> Path:
    ...
    docs = sorted(p.relative_to(docs_dir).as_posix() for p in docs_dir.rglob("*.md") if p.relative_to(docs_dir).as_posix() != "index.md")
...
def status(root: Path = PROJECT_ROOT) -> dict[str, object]:
    ...
    snapshot = {
        ...
        "docs": sorted(p.relative_to(root / "docs").as_posix() for p in (root / "docs").rglob("*.md")) if (root / "docs").exists() else [],
